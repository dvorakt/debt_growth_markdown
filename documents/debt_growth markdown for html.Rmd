...
While reproducible analysis can be done by keeping code and write-up of the analysis separate, new tools now exist that combine the two. One such tool is [RMarkdown](http://rmarkdown.rstudio.com/) which I use to create this note. There is a companion document that performs the same analysis and creates a similar document using [dynamic documents](http://www.stata.com/new-in-stata/markdown/) (`dyndoc`) available in the latest version of Stata (Stata 15). I really don't like dyndoc.
...
#write_csv(wdi,"../original_data/wdi08142017.csv") #save the retrieved data
wdi <- read_csv("../original_data/wdi08142017.csv")
```
